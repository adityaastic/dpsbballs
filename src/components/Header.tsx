        ...
        <div className="flex items-center gap-2">
          ...
          {waNum && (
            <a href={`https://wa.me/${waNum}?text=${waMsg}`}
              ...
              className="hidden lg:inline-flex items-center gap-1.5 px-3 py-2 rounded-full text-white text-[0.72rem] font-bold uppercase tracking-[0.07em] transition-all duration-250 hover:-translate-y-0.5 hover:shadow-lg active:translate-y-0"
              style={{ background: "linear-gradient(135deg,#2dda6e,#18a84d)", boxShadow: "0 4px 16px rgba(37,211,102,0.40)" }}>
              <IconWA />
              <span>WhatsApp</span>
            </a>
          )}
          ...
          {mobile && (
            <a href={`tel:${mobile.replace(/\s/g, "")}`}
              ...
              className="hidden lg:inline-flex items-center gap-1.5 px-3 py-2 rounded-full text-white text-[0.72rem] font-bold uppercase tracking-[0.07em] transition-all duration-250 hover:-translate-y-0.5 hover:shadow-lg active:translate-y-0"
              style={{ background: "linear-gradient(135deg,#ea580c,#7c2d12)", boxShadow: "0 4px 16px rgba(194,65,12,0.38)" }}>
              <IconPhone />
              <span>{mobile}</span>
            </a>
          )}
      ...
      {open && (
        <div className="border-t border-[var(--line)] bg-[var(--surface)] xl:hidden">
          <nav className="mx-auto flex max-w-6xl flex-col px-4 py-3">
            {navLinks.map(link => (
              <Link key={link.href} href={link.href}
                className="border-b border-[var(--line)] py-3.5 text-sm font-semibold text-[var(--ink)] hover:text-[var(--steel)] transition-colors">
                {link.label}
              </Link>
            ))}
            {isAdmin && (
              <Link href="/admin" className="border-b border-[var(--line)] py-3.5 text-sm font-semibold text-[var(--copper)]">
                Admin Panel
              </Link>
            )}
            ...
            <div className="flex gap-2.5 mt-5 mb-2">
              {waNum && (
                <a href={`https://wa.me/${waNum}?text=${waMsg}`} target="_blank" rel="noreferrer"
                  className="flex items-center justify-center gap-2 flex-1 py-3 rounded-xl text-white text-sm font-bold transition-all hover:brightness-110 active:scale-95"
                  style={{ background: "linear-gradient(135deg,#2dda6e,#18a84d)", boxShadow: "0 4px 18px rgba(37,211,102,0.45)" }}>
                  <IconWA />
                  WhatsApp
                </a>
              )}
              {mobile && (
                <a href={`tel:${mobile.replace(/\s/g, "")}`}
                  className="flex items-center justify-center gap-2 flex-1 py-3 rounded-xl text-white text-sm font-bold transition-all hover:brightness-110 active:scale-95"
                  style={{ background: "linear-gradient(135deg,#ea580c,#7c2d12)", boxShadow: "0 4px 18px rgba(194,65,12,0.45)" }}>
                  <IconPhone />
                  Call Now
                </a>
              )}
